create table if not exists public.address_visit_daily_stats (
...
alter table public.address_visit_daily_stats
  add column if not exists stat_date date,
...
create index if not exists address_visit_daily_stats_username_idx
  on public.address_visit_daily_stats (username);

create index if not exists address_visit_daily_stats_stat_date_idx
  on public.address_visit_daily_stats (stat_date desc);

alter table public.address_visit_daily_stats enable row level security;

create table if not exists public.homepage_visit_daily_stats (
...
alter table public.homepage_visit_daily_stats
  add column if not exists stat_date date,
  add column if not exists total_visits integer not null default 0,
  add column if not exists unique_visitors integer not null default 0,
  add column if not exists created_at timestamptz not null default now(),
...
create index if not exists homepage_visit_daily_stats_stat_date_idx
  on public.homepage_visit_daily_stats (stat_date desc);
...
with
  ...
  raw_address_stats as (
    select
      av.visited_at::date as stat_date,
      av.username,
      count(*)::integer as total_visits,
      greatest(
    ...
    where av.username is not null
      ...
      and not exists (
        ...
        where s.stat_date = av.visited_at::date
          and s.username = av.username
      )
    group by av.visited_at::date, av.username
  ),
